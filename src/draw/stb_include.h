   ...
   for (i=0; i < num; ++i) {
      ...
      if (inc_list[i].filename == 0) {
         ...
         if (inc == NULL) {
            ...
            //return NULL;
           // Write the include statement back in
           const char* inclPre = "#include \"";
           text = stb_include_append(text, &textlen,
               inclPre, strlen(inclPre));
           text = stb_include_append(text, &textlen,
               inc_list[i].filename, strlen(inc_list[i].filename));
           const char* inclPost = "\"";
           text = stb_include_append(text, &textlen,
               inclPost, strlen(inclPost));
